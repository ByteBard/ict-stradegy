    ...
    for test_idx in range(train_months, len(unique_months)):
            ...
            for sl, buf, max_hold in param_grid_swing:
                ...
                for val_idx in range(max(0, test_idx - train_months), test_idx):
                    ...
                    ret, nt, nw, _ = backtest_trend_swing(
                        ...
                        sl=sl, swing_buffer=buf, max_hold=max_hold, commission=COST_PER_SIDE)
                    if nt > 0:
                        pnl_per_lot = ret * avg_p * MULTIPLIER
                        n_lots_t = max(1, min(MAX_LOTS, int(INITIAL_CAPITAL / MARGIN_PER_LOT)))
                        month_ret = pnl_per_lot * n_lots_t / INITIAL_CAPITAL
        ...
        for params in top_k_params:
            ...
            if mode == 'trail':
                ...
                sl = params[2]; buf = params[3]; max_hold = params[4]
                sl_adj = sl * sl_scale
                ret_k, n_t_k, n_w_k, _ = backtest_trend_swing(
                    o_arr, c_arr, h_arr, l_arr, sig, last_sh, last_sl,
                    sl=sl_adj, swing_buffer=buf, max_hold=max_hold, commission=COST_PER_SIDE)
            elif mode == 'tstate':
        ...
        # vol_filter: 低波动减仓
        if vol_filter:
            test_key_vol = (test_month, SWING_N_OPTIONS[0])
            if test_key_vol in sig_cache:
                _, _, _, h_v, l_v, ap_v, *_ = sig_cache[test_key_vol]
...
def main():
    ...
    if strategy_list:
        ...
        near_pass = ['S30_bar_quality', 'S26_trend_pullback', 'S6_pd_ote',
                     'S27_ob_pullback', 'S21_mean_reversion']
        # 全10策略
        all_10 = pass_5 + near_pass

        # 1) 5策略 union (基准对照)
        combinations.append((pass_5, 'union'))

        # 2) 10策略 union
        combinations.append((all_10, 'union'))

        # 3) 10策略 quality_union (min_edge=2)
        combinations.append((all_10, 'quality_union'))

        # 4) 10策略 vote (>=3票)
        combinations.append((all_10, 'vote'))
    ...
    baselines = {
        ...
        'S21_mean_reversion': {'ann': 46.0, 'dd': 25.9, 'sr': 1.53, 'wm': 136, 'lm': 53},
    }
